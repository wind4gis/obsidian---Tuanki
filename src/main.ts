...
export class TuankiPlugin extends Plugin {
  ...
  private autoBackupScheduler: import('./services/backup/AutoBackupScheduler').AutoBackupScheduler | null = null;
	...
	// 🌍 初始化国际化系统
	const savedLanguage = (this.settings.language || 'zh-CN') as SupportedLanguage;
	...
	this.addCommand({
		...
		// ❌ 已移除：Open Analytics Dashboard 命令（遵守Obsidian规范，减少不必要的命令）
		// this.addCommand({
		...
		// ❌ 已移除：Open Card Management 命令（遵守Obsidian规范，减少不必要的命令）
		// this.addCommand({
		...
		// ❌ 已移除：Create Subdeck 命令（遵守Obsidian规范，减少不必要的命令）
		// this.addCommand({
		// 	id: "create-subdeck",
		// 	name: "Create Subdeck",
		// 	callback: async () => {
